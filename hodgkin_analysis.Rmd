...
hrscov_path <- "/Users/pourmalm/Documents/Mellinghoff_lab/Data/hodgkin_project/mpIF/tables/intraPatientCoV_PCTHRSStates_v2.2.xlsx"
hrscov_order_path <- "/Users/pourmalm/Documents/Mellinghoff_lab/Data/hodgkin_project/mpIF/tables/hrsStatesHeatmap06.barplot_v2.2.csv"
HRS_agpres_path <- "/Users/pourmalm/Documents/Mellinghoff_lab/Data/hodgkin_project/mpIF/tables/230207_triplePos_B2M,MHCI,MHCII_v1.csv"
hrs_states_path <- "/Users/pourmalm/Documents/Mellinghoff_lab/Data/hodgkin_project/mpIF/tables/Table_S13_HRS cell states_230218.xlsx"
hrs_states_patient_path <- "/Users/pourmalm/Documents/Mellinghoff_lab/Data/hodgkin_project/mpIF/tables/hrs_states_patient.csv"
hrs_states_ptcov_path <- "/Users/pourmalm/Documents/Mellinghoff_lab/Data/hodgkin_project/mpIF/tables/hrs_states_ptcov.csv"
...
HRS_agpres <- read.csv(HRS_agpres_path, header = TRUE, stringsAsFactors = FALSE, sep = ",")
hrs_states_cohort <- read_excel(hrs_states_path, sheet = "Cohort-level_data")
hrs_states_patient <- read.csv(hrs_states_patient_path, header = TRUE, stringsAsFactors = FALSE, sep = ",")
hrs_states_ptcov <- read.csv(hrs_states_ptcov_path, header = TRUE, stringsAsFactors = FALSE, sep = ",")
...
hrsagg_sum2 <- hrsagg %>% 
  ...
  mutate(label_stringent = case_when((percent_HRS_cells_in_aggregate >= 0.3) ~ "aggregated", (percent_HRS_cells_in_aggregate == 0) ~ "not_aggregated")) %>% 
  mutate(label = case_when((percent_HRS_cells_in_aggregate > 0.5) ~ "aggregated", (percent_HRS_cells_in_aggregate == 0) ~ "not_aggregated"))
p <- hrsagg %>% 
  group_by(Patient_ID) %>% 
  summarise(total_N_Cell_Agg = sum(N.Cell.Agg), total_HRS_cells = sum(HRS)) %>% 
  mutate(percent_HRS_cells_in_aggregate = total_N_Cell_Agg/total_HRS_cells) %>% 
  ...
  ggplot(aes(x = factor(Patient_ID, levels = Patient_ID), y = percent_HRS_cells_in_aggregate)) + 
  geom_bar(stat="identity") + 
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  xlab("Patient") + 
  ylab(" % HRS cells in aggregate")

# only EBV- patients
temp <- hrsagg_sum2 %>% filter(label %in% c("aggregated", "not_aggregated")) %>% dplyr::select(Patient_ID)
keep <- intersect(colnames(reformat_rawcounts), temp$Patient_ID)
hrsagg_sum2 <- hrsagg_sum2 %>% 
  filter(Patient_ID %in% keep) %>% 
  column_to_rownames("Patient_ID")
reformat_rawcounts_hrsagg_label <- reformat_rawcounts %>% 
  dplyr::select(keep)
reformat_normcounts_hrsagg_label <- reformat_normcounts %>% 
  dplyr::select(keep)
identical(colnames(reformat_rawcounts_hrsagg_label), rownames(hrsagg_sum2))

# differential expression by 'SampleClustering'
dds_hrsagg_label <- DESeqDataSetFromMatrix(countData=reformat_rawcounts_hrsagg_label, colData=hrsagg_sum2, design=~label)
dds_hrsagg_label$label <- relevel(dds_hrsagg_label$label, "not_aggregated")
dds_hrsagg_label <- DESeq(dds_hrsagg_label)
res_hrsagg_label <- results(dds_hrsagg_label, contrast = c("label", "aggregated", "not_aggregated"))
res_Ordered_hrsagg_label <- res_hrsagg_label[order(res_hrsagg_label$padj),]
res_Ordered_hrsagg_label <- data.frame(GeneID = rownames(res_Ordered_hrsagg_label), res_Ordered_hrsagg_label)

rld <- rlog(dds_hrsagg_label)
...
ggsave("maxcov_orderedbycov.pdf", 
       ...
       height = 200, 
       units = "mm")
# fig 4 correlations, cohort-level fraction CoV
...
p <- total %>% 
  #mutate(color = case_when((Cell_state == "HRS,B2M") ~ "y"), case_when((Cell_state != "HRS,B2M") ~ "n")) %>% 
